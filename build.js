...
<head>
    ...
    <title>Formulir Pengisian CV</title>
    <!-- ExcelJS from CDN -->
...
<div class="container">
    <div class="header" style="display: flex; justify-content: space-between; align-items: flex-start; gap: 0.75rem; flex-wrap: wrap;">
        <div>
            <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap; margin-bottom: 0.25rem;">
                <h1 style="margin: 0; font-size: clamp(1.3rem, 5vw, 2rem); color: var(--text-color);">Formulir Pengisian CV</h1>
                <a href="https://www.linkedin.com/in/fadhlimuhammad/" target="_blank" rel="noopener noreferrer" style="font-size: 0.75rem; background-color: #e8f0fe; color: var(--primary-color); padding: 2px 10px; border-radius: 12px; font-weight: 600; border: 1px solid #cce0ff; display: inline-flex; align-items: center; gap: 4px; text-decoration: none; transition: background-color 0.2s;">
    ...
    <form id="cv-form">
            ...
            <div class="row">
                <div class="col form-group">
                    ...
                    <select id="golongan_darah" name="golongan_darah" required>
                        ...
                        <option value="不明">Tidak Tahu (不明)</option>
                    </select>
                    <div class="error-message">Wajib diisi</div>
                </div>
                <div class="col form-group">
        ...
        <div class="card" id="step-5">
            ...
            <p>Isi dari pendidikan dasar (SD) hingga pendidikan terakhir (Maksimal 4 riwayat).</p>
            ...
            <div class="form-group">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; gap: 0.5rem; flex-wrap: wrap;">
                    ...
                    <div style="display: flex; gap: 6px; flex-wrap: wrap;">
                        ...
                        <button type="button" class="btn-secondary" onclick="openKepribadianModal(this)" style="font-size: 0.75rem; padding: 0.25rem 0.5rem; white-space: nowrap; border-color: #34a853; color: #34a853; display: flex; align-items: center; gap: 4px;">
                            💡 Tanya Asisten (Formal)
                        </button>
                    </div>
                </div>
    ...
    <footer style="text-align: center; margin-top: 2rem; margin-bottom: 1.5rem; font-size: 0.85rem; color: #5f6368; border-top: 1px solid var(--border-color); padding-top: 1.25rem;">
        <p style="margin: 0; font-weight: 500; color: #3c4043;">Formulir Pengisian CV</p>
        <p style="margin: 0.35rem 0 0 0; font-size: 0.8rem; color: #70757a;">
            Watermark: <a href="https://www.linkedin.com/in/fadhlimuhammad/" target="_blank" rel="noopener noreferrer" style="color: var(--primary-color); font-weight: 600; text-decoration: none;">by Fadhli Muhammad</a>
        </p>
    </footer>
</div>

<a href="https://www.linkedin.com/in/fadhlimuhammad/" target="_blank" rel="noopener noreferrer" class="watermark-floating" style="position: fixed; bottom: 12px; right: 16px; font-size: 0.75rem; color: #5f6368; background: rgba(255, 255, 255, 0.92); padding: 5px 12px; border-radius: 20px; border: 1px solid var(--border-color); box-shadow: 0 2px 6px rgba(0,0,0,0.08); z-index: 999; backdrop-filter: blur(4px); font-weight: 500; display: flex; align-items: center; gap: 6px; text-decoration: none; transition: transform 0.2s, box-shadow 0.2s;">
    <span style="display: inline-block; width: 7px; height: 7px; background-color: var(--primary-color); border-radius: 50%;"></span>
    <span>by Fadhli Muhammad</span>
</a>
...
<script>
    ...
    // WORK DATE FORMATTER HELPER
    function formatWorkDate(year, month) {
        if (!year && !month) return '';
        const y = year ? String(year).trim().replace(/年$/, '') : '';
        const m = month ? String(month).trim().replace(/月$/, '') : '';
        if (y && m) {
            return \`\${y}年\${m}月\`;
        }
        if (y) {
        ...
        for (let i = 1; i <= 4; i++) {
            workHtml += \`
                <div class="repeated-group" id="pekerjaan_group_\${i}" style="\${i > 1 ? 'display: none;' : ''}">
                    ...
                    <div class="row">
                        <div class="col form-group">
                            <label>Waktu Mulai Bekerja</label>
                            <div style="display: flex; gap: 8px;">
                                <select id="work_start_month_\${i}" name="work_start_month_\${i}" style="flex: 1.2;">
                                ...
                                <input type="number" id="work_start_year_\${i}" name="work_start_year_\${i}" placeholder="Tahun (cth: 2021)" min="1970" max="2099" style="flex: 1;">
                            </div>
                        </div>
                        <div class="col form-group">
                            <label>Waktu Selesai Bekerja</label>
                            <div style="display: flex; gap: 8px;">
                                <select id="work_end_month_\${i}" name="work_end_month_\${i}" style="flex: 1.2;">
                                    <option value="">Bulan...</option>
                                    <option value="1">1 (Januari)</option>
                                    <option value="2">2 (Februari)</option>
                                    <option value="3">3 (Maret)</option>
                                ...
                                <input type="number" id="work_end_year_\${i}" name="work_end_year_\${i}" placeholder="Tahun (cth: 2023)" min="1970" max="2099" style="flex: 1;">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col form-group">
                            ...
                            <input type="text" id="work_company_\${i}" name="work_company_\${i}" placeholder="Contoh: PT MAJU MUNDUR">
                        </div>
                        <div class="col form-group">
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; gap: 0.5rem; flex-wrap: wrap;">
                            ...
                            <input type="text" id="work_job_\${i}" name="work_job_\${i}" placeholder="contoh: 溶接工 atau 社員">
                        </div>
                    </div>
                </div>
            \`;
    ...
    function loadFromLocalStorage() {
        ...
        if (savedData) {
            ...
            // Backward compatibility for old work_start_i and work_end_i
            for (let i = 1; i <= 4; i++) {
                if (data[\`work_start_\${i}\`] && !data[\`work_start_year_\${i}\`]) {
                    const str = String(data[\`work_start_\${i}\`]).trim();
                    const match = str.match(/(\\d{4})/);
                    if (match) {
                        const yearEl = document.getElementById(\`work_start_year_\${i}\`);
                        if (yearEl) yearEl.value = match[1];
                    }
                }
                if (data[\`work_end_\${i}\`] && !data[\`work_end_year_\${i}\`]) {
                    const str = String(data[\`work_end_\${i}\`]).trim();
                    const match = str.match(/(\\d{4})/);
                    if (match) {
                        const yearEl = document.getElementById(\`work_end_year_\${i}\`);
                        if (yearEl) yearEl.value = match[1];
                    }
                }
            }
            ...
            for (let i = 2; i <= 4; i++) {
                if (data[\`work_company_\${i}\`] || data[\`work_start_year_\${i}\`] || data[\`work_start_\${i}\`]) {
    ...
    function removeKerja(index) {
        for (let i = index; i < currentKerjaCount; i++) {
            const nextI = i + 1;
            document.getElementById(\`work_start_month_\${i}\`).value = document.getElementById(\`work_start_month_\${nextI}\`).value;
            document.getElementById(\`work_start_year_\${i}\`).value = document.getElementById(\`work_start_year_\${nextI}\`).value;
            document.getElementById(\`work_end_month_\${i}\`).value = document.getElementById(\`work_end_month_\${nextI}\`).value;
            document.getElementById(\`work_end_year_\${i}\`).value = document.getElementById(\`work_end_year_\${nextI}\`).value;
        ...
        document.getElementById(\`work_start_month_\${lastI}\`).value = '';
        document.getElementById(\`work_start_year_\${lastI}\`).value = '';
        document.getElementById(\`work_end_month_\${lastI}\`).value = '';
        document.getElementById(\`work_end_year_\${lastI}\`).value = '';
    ...
    function generatePreview() {
        ...
        function renderSectionHeader(title, step) {
            return \`
                <div style="display: flex; justify-content: space-between; align-items: center; margin: 1.5rem 0 0.5rem; padding-bottom: 0.5rem; border-bottom: 2px solid var(--primary-color);">
                    <h3 style="margin: 0; color: var(--primary-color); font-size: 1.1rem;">\${title}</h3>
                    <button type="button" class="btn-secondary" onclick="goToStep(\${step})" style="font-size: 0.8rem; padding: 0.25rem 0.6rem; border-color: #1a73e8; color: #1a73e8; display: inline-flex; align-items: center; gap: 4px; cursor: pointer; border-radius: 4px;">
        ...
        // Step 1: Informasi Dasar
        html += renderSectionHeader('1. Informasi Dasar', 1);
        ...
        html += \`<tr><th>Tanggal Lahir / Umur</th><td>\${jpDate ? jpDate + ' / ' : ''}\${data.umur ? data.umur + '歳' : '-'}</td></tr>\`;
        html += \`<tr><th>Golongan Darah</th><td>\${data.golongan_darah || '-'}</td></tr>\`;
        html += \`<tr><th>Status Pernikahan</th><td>\${data.status_pernikahan || '-'}</td></tr>\`;
        html += '</tbody></table>';
        html += '</div>';

        // Step 2: Fisik & Kesehatan
        html += renderSectionHeader('2. Fisik & Kesehatan', 2);
        html += '<div class="table-responsive">';
        html += '<table class="preview-table"><tbody>';
        ...
        const tanganText = data.tangan_dominan === '右' ? 'Kanan (右)' : (data.tangan_dominan === '左' ? 'Kiri (左)' : (data.tangan_dominan || '-'));
        html += \`<tr><th>Tangan Dominan</th><td>\${tanganText}</td></tr>\`;
        html += \`<tr><th>Merokok / Minum Alkohol</th><td>\${data.merokok || '-'} / \${data.alkohol || '-'}</td></tr>\`;
        html += \`<tr><th>Riwayat Penyakit</th><td>\${data.riwayat_penyakit || '-'}</td></tr>\`;
        html += '</tbody></table>';
        html += '</div>';

        // Step 3: Alamat & Kontak
        html += renderSectionHeader('3. Alamat & Kontak', 3);
        html += '<div class="table-responsive">';
        html += '<table class="preview-table"><tbody>';
        html += \`<tr><th>Alamat Lengkap</th><td>\${data.alamat || '-'}</td></tr>\`;
        html += \`<tr><th>Nomor HP / WhatsApp</th><td>\${data.no_hp || '-'}</td></tr>\`;
        html += '</tbody></table>';
        html += '</div>';

        // Step 4: Latar Belakang & Motivasi
        html += renderSectionHeader('4. Latar Belakang & Motivasi', 4);
        html += '<div class="table-responsive">';
        html += '<table class="preview-table"><tbody>';
        html += \`<tr><th>Bidang Keahlian</th><td>\${data.mata_pelajaran || '-'}</td></tr>\`;
        html += \`<tr><th>Pengalaman Asrama</th><td>\${data.pengalaman_asrama || '-'}</td></tr>\`;
        html += \`<tr><th>Pendapatan Saat Ini</th><td>\${data.pendapatan || '-'}</td></tr>\`;
        html += \`<tr><th>Target Tabungan</th><td>\${data.target_tabungan || '-'}</td></tr>\`;
        html += \`<tr><th>Alasan / Motivasi</th><td style="white-space: pre-wrap;">\${data.alasan || '-'}</td></tr>\`;
        html += \`<tr><th>Pekerjaan Setelah Pulang</th><td style="white-space: pre-wrap;">\${data.pekerjaan_nanti || '-'}</td></tr>\`;
        html += \`<tr><th>Pengalaman Visa Jepang</th><td>\${data.pengalaman_visa || '-'}</td></tr>\`;
        html += '</tbody></table>';
        html += '</div>';

        // Step 5: Education
        html += renderSectionHeader('5. Riwayat Pendidikan', 5);
        html += '<div class="table-responsive">';
        ...
        if (!hasEdu) html += '<tr><td colspan="3" style="text-align:center;">Tidak ada riwayat pendidikan</td></tr>';
        html += '</tbody></table>';
        html += '</div>';

        // Step 6: Work
        html += renderSectionHeader('6. Riwayat Pekerjaan', 6);
        html += '<div class="table-responsive">';
        ...
        for (let i = 1; i <= 4; i++) {
            if (data[\`work_company_\${i}\`] || data[\`work_start_year_\${i}\`] || data[\`work_start_\${i}\`]) {
                hasWork = true;
                const startDate = formatWorkDate(data[\`work_start_year_\${i}\`] || data[\`work_start_\${i}\`], data[\`work_start_month_\${i}\`]);
                const endDate = formatWorkDate(data[\`work_end_year_\${i}\`] || data[\`work_end_\${i}\`], data[\`work_end_month_\${i}\`]);
        ...
        // Step 7: Family
        html += renderSectionHeader('7. Anggota Keluarga', 7);
        html += '<div class="table-responsive">';
        ...
        for (let i = 1; i <= 15; i++) {
            if (data[\`fam_name_\${i}\`]) {
                ...
                html += \`<tr>
                    ...
                    <td>\${data[\`fam_age_\${i}\`] ? data[\`fam_age_\${i}\`] + '歳' : '-'}</td>
                    <td>\${data[\`fam_job_\${i}\`] || '-'}</td>
                    <td>\${data[\`fam_together_\${i}\`] === '〇' ? 'Ya (〇)' : 'Tidak'}</td>
                </tr>\`;
            }
        }
        if (!hasFam) html += '<tr><td colspan="5" style="text-align:center;">Tidak ada data anggota keluarga</td></tr>';
        html += '</tbody></table>';
        html += '</div>';

        // Step 8: Lain-lain
        html += renderSectionHeader('8. Lain-lain (Hobi, Kepribadian, Komitmen)', 8);
        html += '<div class="table-responsive">';
        html += '<table class="preview-table"><tbody>';
        html += \`<tr><th>Hobi & Keahlian</th><td style="white-space: pre-wrap;">\${data.hobi || '-'}</td></tr>\`;
        html += \`<tr><th>Kelebihan & Kekurangan</th><td style="white-space: pre-wrap;">\${data.kepribadian || '-'}</td></tr>\`;
        html += \`<tr><th>Komitmen saat Magang</th><td style="white-space: pre-wrap;">\${data.komitmen || '-'}</td></tr>\`;
        html += '</tbody></table>';
        html += '</div>';
    ...
    async function generateExcelFile() {
        ...
        for(let i=1; i<=4; i++) {
            ...
            if (data[\`work_company_\${i}\`] || data[\`work_start_year_\${i}\`] || data[\`work_start_\${i}\`]) {
                const startDate = formatWorkDate(data[\`work_start_year_\${i}\`] || data[\`work_start_\${i}\`], data[\`work_start_month_\${i}\`]);
                const endDate = formatWorkDate(data[\`work_end_year_\${i}\`] || data[\`work_end_\${i}\`], data[\`work_end_month_\${i}\`]);
                updateCell(\`A\${row}\`, startDate);
                updateCell(\`D\${row}\`, endDate);
    ...
    function fillFakeData() {
        ...
        const dummyData = {
            ...
            edu_start_4: "", edu_end_4: "", edu_school_4: "", edu_cert_4: "卒業証明書",
            work_start_month_1: "4", work_start_year_1: "2021", work_end_month_1: "3", work_end_year_1: "2023", work_company_1: "PT MAJU MUNDUR", work_job_1: "溶接工",
            work_start_month_2: "4", work_start_year_2: "2023", work_end_month_2: "5", work_end_year_2: "2025", work_company_2: "PT SEJAHTERA", work_job_2: "溶接工",
            work_start_month_3: "", work_start_year_3: "", work_end_month_3: "", work_end_year_3: "", work_company_3: "", work_job_3: "",
            work_start_month_4: "", work_start_year_4: "", work_end_month_4: "", work_end_year_4: "", work_company_4: "", work_job_4: "",
    ...
    function resetForm() {
        ...
        // Reset undo history
        document.querySelectorAll("[id^='btn_undo_']").forEach(btn => btn.remove());
        for (let key in fieldUndoHistory) {
            delete fieldUndoHistory[key];
        }
    ...
    // UNDO TRANSLATION SYSTEM
    const fieldUndoHistory = {};

    function saveForUndo(fieldId) {
        const el = document.getElementById(fieldId);
        if (el) {
            fieldUndoHistory[fieldId] = el.value;
        }
    }

    function undoTranslation(fieldId) {
        const el = document.getElementById(fieldId);
        if (el && fieldUndoHistory[fieldId] !== undefined) {
            const currentVal = el.value;
            el.value = fieldUndoHistory[fieldId];
            fieldUndoHistory[fieldId] = currentVal;
    ...
    function showUndoButton(fieldId, anchorBtn) {
        let undoBtn = document.getElementById(\`btn_undo_\${fieldId}\`);
        if (!undoBtn) {
            undoBtn = document.createElement('button');
            undoBtn.type = 'button';
            undoBtn.id = \`btn_undo_\${fieldId}\`;
            undoBtn.className = 'btn-secondary';
            undoBtn.style.cssText = 'font-size: 0.75rem; padding: 0.25rem 0.5rem; white-space: nowrap; border-color: #ea4335; color: #ea4335; display: inline-flex; align-items: center; gap: 4px; border-radius: 4px; cursor: pointer; transition: all 0.2s;';
            undoBtn.title = 'Kembalikan teks asli sebelum diterjemahkan';
            undoBtn.innerHTML = \`
            ...
            undoBtn.onclick = function() {
                undoTranslation(fieldId);
            };
            
            if (anchorBtn && anchorBtn.parentNode) {
                anchorBtn.parentNode.insertBefore(undoBtn, anchorBtn.nextSibling);
            }
        } else {
